...
cfe_MC=cL*(Ccalo_MC/cL+me_MC)/mfe_MC*(TF2_MC-T3_MC)/(T4_MC-TF2_MC)
moycfe=np.average(cfe_MC)
ucfe=np.std(cfe_MC,ddof=1)

print('\033[0;33m la capacité thermique massique du fer est cfe',moycfe,'J.g^-1.K^-1 \n et l incertitude associée est', ucfe, 'J.g^-1.K^-1. \033[0m')
...
D_MC=cL*(-TF3_MC+(ml_MC+Ccalo_MC/cL)/ms_MC*(T5_MC-TF3_MC))+cS*T6_MC
moyD=np.average(D_MC)
uD=np.std(D_MC,ddof=1)
...
print('L évaluation de Tau est :',moyTau,'s \n et celle sur l incertitude est u(Tau)',uTau,'s')

R_MC=R+rd.uniform(-lR,lR,N)
moyR=np.average(R_MC)
uR=np.std(R_MC,ddof=1)
print('L évaluation de R est :',moyR,'s \n et celle sur l incertitude est u(R)',uR,'Ohm')
#on évalue la capacité thermique du calorimètre
Ccal_MC=(U_MC**2*Tau_MC)/R/(TF4_MC-T7_MC)-cL*m_MC
moyCcal=np.average(Ccal_MC)
uCcal=np.std(Ccal_MC,ddof=1)

print('\033[0;33m la capacité thermique du calorimètre est Ccalo',moyCcal,'J.kg^-1 \n et l incertitude associée est', uCcal, 'J.K^-1. \033[0m')
